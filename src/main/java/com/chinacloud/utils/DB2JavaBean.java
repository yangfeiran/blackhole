...
public class DB2JavaBean {
	...
	private String url = "jdbc:mysql://172.16.50.21/blackhole"; // 数据库访问串
	private String userName = "root"; // 数据库用户名
	private String password = "654321"; // 数据库密码
	...
	public void init() {
		try {
			...
			for (int i = 0; i < tables.length; i++) {
				...
				String pkg = outputDir.split("src/main/java/")[outputDir.split("src/main/java/").length - 1];
				pkg = pkg.replaceAll("/", ".");
	...
	private String getFieldStr(String strType, String strName, String keyName) {
		...
		if(strName.equals(keyName)){
			str += "@Id";
			if(("Integer".equals(strType) || "Long".equals(strType))){
				str += "\n\t";
				str += "@GeneratedValue(strategy = GenerationType.AUTO)";
			}
		}else 
		    str += "@Column(name=\""+strName+"\")";
		str +="\n\t";
	...
	public static void main(String[] args) {
		...
		d2j.setTableName("test");
		d2j.setOutputDir("src/main/java/com/chinacloud/model");
		d2j.init();
